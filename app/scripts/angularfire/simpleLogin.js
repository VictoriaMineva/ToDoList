  ...
  angular.module('simpleLogin', ['firebase', 'firebase.utils', 'firebase.config'])
    ...
    .factory('simpleLogin', function($firebaseAuth, fbutil, $q, $rootScope, createProfile) {
      ...
      var fns = {
        ...
        createAccount: function(email, pass, name, opts) {
          return auth.$createUser({email: email, password: pass, name: name})
            .then(function() {
              // authenticate so we have permission to write to Firebase
              return fns.passwordLogin({email: email, password: pass, name: name}, opts);
            })
            .then(function(user) {
